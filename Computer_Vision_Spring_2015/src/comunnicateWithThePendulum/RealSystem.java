package comunnicateWithThePendulum;
...
public class RealSystem extends Thread {
	...
	// THESE VALUES ARE TAKEN FROM THE SIMULINK BLOCK "Real_system" IN THE COURSE FRTN05 FOR THE PROCESS IN THE 
	private final double ACCE_IN_GAIN = 1000/Math.pow(2, 15) ;
	private final double VEL_IN_GAIN = Math.pow(2, 5)*1000/Math.pow(2, 17);
	// END VALUES
	...
	public void run(){
		...
		while(weShouldRun){
			...
			acceMon.setAcce(   limitAcce(acceMon.getAcce())*ACCE_IN_GAIN );		// Limiting the acce
			try {
			...
			try {
				analogOutVelocity.set(this.vel_state*VEL_IN_GAIN);
			} catch (IOChannelException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
