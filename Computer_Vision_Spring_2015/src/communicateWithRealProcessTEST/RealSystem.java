package communicateWithRealProcessTEST;
	...
	public void run(){
		...
		while(weShouldRun){
			synchronized(acceMon){
				...
				try {
					analogOutAcceleration.set(acceMon.getAcce()*ACCE_TO_PROCESS);
				...
				try {
					analogOutVelocity.set((this.vel_state*VELOCITY_TO_PROCESS));
